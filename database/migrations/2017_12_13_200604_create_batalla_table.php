...
class CreateBatallaTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('batalla', function (Blueprint $table) {
            $table->increments('idBatalla');
            $table->integer('idEntrenadorHumano')->unsigned();
            $table->foreign('idEntrenadorHumano')->references('idEntrenador')->on('entrenador');
            $table->integer('idEntrenadorArtificial')->unsigned();
            $table->foreign('idEntrenadorArtificial')->references('idEntrenador')->on('entrenador');
